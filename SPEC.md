...
9. **Box office basis travels with the fact.** `figures_basis` is required when `film.gross` exists, and a year marked `unavailable` may not carry dollar amounts. The **territory** must also be named in the fact's label or detail, in the words "North America", "domestic", "United States and Canada", or "worldwide". `figures_basis` cannot express it, and the difference is larger than the rentals-to-gross one: 1988 and 1989 rank by worldwide gross while 1987 ranks by North American gross, so a reader comparing the biggest film of adjacent years is comparing different measurements unless the fact says which. This is checked rather than trusted because the pages vary year to year with no pattern, exactly as the basis does.
...
2. **Distinctive-token density.** Every texture paragraph needs at least two distinctive tokens, where a distinctive token is a proper noun or a specific number that appears in at most three year files across the corpus. A 2011 paragraph mentioning only smartphones and social media fails. One mentioning Qwikster and Fukushima passes.
   ...
   This check is corpus-relative, so a passing run is not a permanent verdict. Filling in more years makes tokens commoner, and a paragraph that passed when it was written can fail later without being touched: 1929 broke this way once the corpus reached fifty-four years. That is the check working correctly, because a name in five files really is less distinctive than one in two, and the fix is to give the paragraph a sharper detail rather than to loosen the threshold. A paragraph sitting exactly on two tokens therefore **warns**, naming the tokens it depends on, so the fragility is visible a batch early instead of arriving as a failure. Warnings do not fail the gate.
